...
class TestRolling:
    ...
    def test_rolling_apply(self, raw, roll_frame):
        ...
        # reduction
        result = r.apply(lambda x: x.add(), raw=raw)
        expected = g.apply(lambda x: x.rolling(4).apply(lambda y: y.add(), raw=raw))
        # groupby.apply doesn't drop the grouped-by column
        expected = expected.drop("A", axis=1)
        # GH 39732
        expected_index = MultiIndex.from_arrays([roll_frame["A"], range(40)])
    ...
    def test_groupby_rolling_custom_indexer(self):
        ...
        expected = df.groupby(df.index).rolling(window=3, min_periods=1).add()
        tm.assert_frame_equal(result, expected)
    ...
    def test_groupby_rolling_subset_with_closed(self):
        ...
        result = (
            df.groupby("group").rolling("1D", on="date", closed="left")["column1"].add()
        )
        expected = Series(
    ...
    def test_groupby_rolling_empty_frame(self):
        ...
        result = expected.groupby("s1").rolling(window=1).add()
        # GH 32262
        ...
        result = expected.groupby(["s1", "s2"]).rolling(window=1).add()
        # GH 32262
    ...
    def test_groupby_rolling_resulting_multiindex2(self):
        ...
        result = df.groupby(["b", "c"]).rolling(2).add()
        expected_index = MultiIndex.from_tuples(
            [
                (1, 1, 0),
                (1, 1, 4),
    ...
    def test_groupby_rolling_object_doesnt_affect_groupby_apply(self, roll_frame):
        # GH 39732
        g = roll_frame.groupby("A", group_keys=False)
        expected = g.apply(lambda x: x.rolling(4).add()).index
        _ = g.rolling(window=4)
        result = g.apply(lambda x: x.rolling(4).add()).index
    ...
    def test_groupby_level(self):
        ...
        result = df.groupby(level=0)["Max Speed"].rolling(2).add()
        expected = Series(
    ...
    def test_groupby_monotonic(self):
        ...
        expected = (
            df.set_index("date")
            .groupby("name")
            .apply(lambda x: x.rolling("180D")["amount"].add())
        )
        result = df.groupby("name").rolling("180D", on="date")["amount"].add()
        tm.assert_series_equal(result, expected)
...
class TestExpanding:
    ...
    @pytest.mark.parametrize(
        "f", ["add", "mean", "min", "max", "count", "kurt", "skew"]
    )
    ...
    def test_expanding_apply(self, raw):
        ...
        # reduction
        result = r.apply(lambda x: x.add(), raw=raw)
        expected = g.apply(lambda x: x.expanding().apply(lambda y: y.add(), raw=raw))
        # groupby.apply doesn't drop the grouped-by column
        expected = expected.drop("A", axis=1)
        # GH 39732
        expected_index = MultiIndex.from_arrays([self.frame["A"], range(40)])
