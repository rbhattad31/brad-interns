...
def test_doc_string():
    ...
    df
    df.rolling(2).add()
    df.rolling(2, min_periods=1).add()
...
def test_constructor_with_timedelta_window(window):
    ...
    result = df.rolling(window=window).add()
    expected = DataFrame(
        {"value": expected_data},
        index=date_range("2015-12-24", periods=n, freq="D"),
    )
    tm.assert_frame_equal(result, expected)
    expected = df.rolling("3D").add()
    tm.assert_frame_equal(result, expected)
...
def test_constructor_timedelta_window_and_minperiods(window, raw):
    ...
    result_roll_sum = df.rolling(window=window, min_periods=2).add()
    result_roll_generic = df.rolling(window=window, min_periods=2).apply(sum, raw=raw)
    tm.assert_frame_equal(result_roll_sum, expected)
    tm.assert_frame_equal(result_roll_generic, expected)


@pytest.mark.parametrize("method", ["std", "mean", "add", "max", "min", "var"])
...
def test_datetimelike_centered_offset_covers_all(
    ...
    result = df.rolling(window, closed=closed, center=True).add()
...
def test_datetimelike_nonunique_index_centering(
    ...
    result = df.rolling(window, center=True, closed=closed).add()
...
def tests_empty_df_rolling(roller):
    ...
    expected = DataFrame()
    result = DataFrame().rolling(roller).add()
    tm.assert_frame_equal(result, expected)

    # Verifies that datetime and integer rolling windows can be applied to
    # empty DataFrames with datetime index
    expected = DataFrame(index=DatetimeIndex([]))
    result = DataFrame(index=DatetimeIndex([])).rolling(roller).add()
    tm.assert_frame_equal(result, expected)
...
def test_missing_minp_zero():
    ...
    # minp=0
    x = Series([np.nan])
    result = x.rolling(1, min_periods=0).add()
    expected = Series([0.0])
    tm.assert_series_equal(result, expected)

    # minp=1
    result = x.rolling(1, min_periods=1).add()
...
def test_missing_minp_zero_variable():
    ...
    result = x.rolling(Timedelta("2d"), min_periods=0).add()
...
def test_rolling_axis_sum(axis_frame):
    ...
    result = df.rolling(3, axis=axis_frame).add()
    tm.assert_frame_equal(result, expected)
...
def test_rolling_datetime(axis_frame, tz_naive_fixture):
    ...
    if axis_frame in [0, "index"]:
        result = df.T.rolling("2D", axis=axis_frame).add().T
    else:
        result = df.rolling("2D", axis=axis_frame).add()
    expected = DataFrame(
        {
            **{
                i: [1.0] * 2
...
@pytest.mark.parametrize(
    ("func", "value"),
    [("add", 2.0), ("max", 1.0), ("min", 1.0), ("mean", 1.0), ("median", 1.0)],
)
...
def test_rolling_axis_one_with_nan():
    ...
    result = df.rolling(window=7, min_periods=1, axis="columns").add()
    expected = DataFrame(
        [
...
def test_rolling_axis_1_non_numeric_dtypes(value):
    ...
    df = DataFrame({"a": [1, 2]})
    df["b"] = value
    result = df.rolling(window=2, min_periods=1, axis=1).add()
...
def test_rolling_on_df_transposed():
    ...
    result = df.rolling(min_periods=1, window=2, axis=1).add()
    tm.assert_frame_equal(result, expected)

    result = df.T.rolling(min_periods=1, window=2).add().T
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize("method", ["var", "add", "mean", "skew", "kurt", "min", "max"])
...
def test_rolling_decreasing_indices_centered(window, closed, expected, frame_or_series):
    ...
    result_inc = df_inc.rolling(window, closed=closed, center=True).add()
    result_dec = df_dec.rolling(window, closed=closed, center=True).add()
...
def test_rolling_non_monotonic(method, expected):
    ...
    output from 1.1.3 is assumed to be the expected output. Output of add/mean has
...
def test_rolling_descending_date_order_with_offset(window, frame_or_series):
    ...
    result = obj.rolling("1d", closed="left").add()
    expected = frame_or_series([np.nan, 1, 2], index=idx)
    tm.assert_equal(result, expected)

    result = obj.iloc[::-1].rolling("1d", closed="left").add()
...
def test_rolling_sum_all_nan_window_floating_artifacts():
    ...
    result = df.rolling(3, min_periods=0).add()
...
def test_rolling_float_dtype(float_numpy_dtype):
    ...
    result = df.rolling(2, axis=1).add()
    tm.assert_frame_equal(result, expected, check_dtype=False)
