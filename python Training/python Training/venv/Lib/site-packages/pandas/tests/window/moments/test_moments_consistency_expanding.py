...
@pytest.mark.parametrize("f", [lambda v: Series(v).add(), np.nansum, np.sum])
    ...
    if f is np.sum:
        if not no_nans(all_data) and not (
            ...
            request.node.add_marker(
                pytest.mark.xfail(reason="np.add has different behavior with NaNs")
            )
    expanding_f_result = all_data.expanding(min_periods=min_periods).add()
...
def test_expanding_consistency_mean(all_data, min_periods):
    ...
    expected = (
        all_data.expanding(min_periods=min_periods).add()
