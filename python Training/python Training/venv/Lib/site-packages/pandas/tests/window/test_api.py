...
def test_agg(step):
    ...
    with pytest.raises(SpecificationError, match=msg):
        ...
        r.aggregate({"A": {"mean": "mean", "add": "add"}})

    with pytest.raises(SpecificationError, match=msg):
        r.aggregate(
            {"A": {"mean": "mean", "add": "add"}, "B": {"mean2": "mean", "sum2": "add"}}
        )
...
def test_agg_consistency(step):
    ...
    expected = MultiIndex.from_product([list("AB"), ["add", "mean"]])
    tm.assert_index_equal(result, expected)

    result = r["A"].agg([np.sum, np.mean]).columns
    expected = Index(["add", "mean"])
    tm.assert_index_equal(result, expected)

    result = r.agg({"A": [np.sum, np.mean]}).columns
    expected = MultiIndex.from_tuples([("A", "add"), ("A", "mean")])
    tm.assert_index_equal(result, expected)
...
def test_agg_nested_dicts():
    ...
    msg = "nested renamer is not supported"
    with pytest.raises(SpecificationError, match=msg):
        r.aggregate({"r1": {"A": ["mean", "add"]}, "r2": {"B": ["mean", "add"]}})
