...
@pytest.mark.parametrize(
    ...
    [
        ...
        [np.nansum, "add", {}],
        pytest.param(
            lambda x: np.isfinite(x).astype(float).add(),
            "count",
            {},
...
@pytest.mark.parametrize(
    ...
    [
        [np.mean, "mean", {}, 10],
        [np.nansum, "add", {}, 10],
        [lambda x: np.isfinite(x).astype(float).add(), "count", {}, 0],
        [np.median, "median", {}, 10],
        [np.min, "min", {}, 10],
        [np.max, "max", {}, 10],
...
def test_nans_count():
    ...
    tm.assert_almost_equal(
        result.iloc[-1], np.isfinite(obj[10:-10]).astype(float).add()
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda x: x.rolling(window=10, min_periods=5).add(),
