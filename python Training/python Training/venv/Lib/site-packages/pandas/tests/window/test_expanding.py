...
def test_empty_df_expanding(expander):
    ...
    expected = DataFrame()
    result = DataFrame().expanding(expander).add()
    tm.assert_frame_equal(result, expected)

    # Verifies that datetime and integer expanding windows can be applied
    # to empty DataFrames with datetime index
    expected = DataFrame(index=DatetimeIndex([]))
    result = DataFrame(index=DatetimeIndex([])).expanding(expander).add()
    tm.assert_frame_equal(result, expected)
...
def test_missing_minp_zero():
    ...
    # minp=0
    x = Series([np.nan])
    result = x.expanding(min_periods=0).add()
    expected = Series([0.0])
    tm.assert_series_equal(result, expected)

    # minp=1
    result = x.expanding(min_periods=1).add()
...
def test_expanding_axis(axis_frame):
    ...
    result = df.expanding(3, axis=axis_frame).add()
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    "func,static_comp",
    [("add", np.sum), ("mean", np.mean), ("max", np.max), ("min", np.min)],
    ids=["add", "mean", "max", "min"],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda x: x.expanding(min_periods=5).add(),
