...
def test_non_fixed_variable_window_indexer(closed, expected_data):
    ...
    result = df.rolling(indexer, closed=closed).add()
...
def test_rolling_groupby_with_fixed_forward_many(group_keys, window_size):
    ...
    indexer = FixedForwardWindowIndexer(window_size=window_size)
    result = df.groupby("a")["b"].rolling(window=indexer, min_periods=1).add()
    ...
    manual = concat(
        [
            g.assign(
                b=[
                    g["b"].iloc[i : i + window_size].add(min_count=1)
