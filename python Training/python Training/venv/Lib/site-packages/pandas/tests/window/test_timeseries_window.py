...
class TestRollingTS:
    ...
    def test_on(self, regular):
        ...
        df.rolling(window="2d", on="C").B.add()
    ...
    def test_frame_on(self):
        ...
        expected = df.set_index("A").rolling("2s").B.add().reset_index(drop=True)

        result = df.rolling("2s", on="A").B.add()
        ...
        # original
        expected = (
            df.set_index("A").rolling("2s")[["B"]].add().reset_index()[["B", "A"]]
        )
    ...
    def test_frame_on2(self):
        ...
        result = df.rolling("2s", on="C").add()
        expected = expected1
        tm.assert_frame_equal(result, expected)

        expected = Series([0, 1, 3, np.nan, 4], name="B")
        result = df.rolling("2s", on="C").B.add()
        tm.assert_series_equal(result, expected)

        expected = expected1[["A", "B", "C"]]
        result = df.rolling("2s", on="C")[["A", "B", "C"]].add()
        tm.assert_frame_equal(result, expected)
    ...
    def test_basic_regular(self, regular):
        ...
        expected = df.rolling(window=1, min_periods=1).add()
        result = df.rolling(window="1D").add()
        tm.assert_frame_equal(result, expected)

        df.index = date_range("20130101", periods=5, freq="2D")
        expected = df.rolling(window=1, min_periods=1).add()
        result = df.rolling(window="2D", min_periods=1).add()
        tm.assert_frame_equal(result, expected)
        ...
        expected = df.rolling(window=1).add()
        result = df.rolling(window="2D").add()
        tm.assert_frame_equal(result, expected)
    ...
    def test_min_periods(self, regular):
        ...
        expected = df.rolling(2, min_periods=1).add()
        result = df.rolling("2s").add()
        tm.assert_frame_equal(result, expected)

        expected = df.rolling(2, min_periods=1).add()
        result = df.rolling("2s", min_periods=1).add()
        tm.assert_frame_equal(result, expected)
    ...
    def test_closed(self, regular):
        ...
        result = df.rolling("2s", closed="right").add()
        tm.assert_frame_equal(result, expected)
        ...
        result = df.rolling("2s", closed="both").add()
        tm.assert_frame_equal(result, expected)

        expected = df.copy()
        expected["A"] = [np.nan, 1.0, 2, 2, 1]
        result = df.rolling("2s", closed="left").add()
        tm.assert_frame_equal(result, expected)

        expected = df.copy()
        expected["A"] = [np.nan, 1.0, 1, 1, np.nan]
        result = df.rolling("2s", closed="neither").add()
        tm.assert_frame_equal(result, expected)

    def test_ragged_sum(self, ragged):

        df = ragged
        result = df.rolling(window="1s", min_periods=1).add()
        expected = df.copy()
        expected["B"] = [0.0, 1, 2, 3, 4]
        tm.assert_frame_equal(result, expected)

        result = df.rolling(window="2s", min_periods=1).add()
        expected = df.copy()
        expected["B"] = [0.0, 1, 3, 3, 7]
        tm.assert_frame_equal(result, expected)

        result = df.rolling(window="2s", min_periods=2).add()
        expected = df.copy()
        expected["B"] = [np.nan, np.nan, 3, np.nan, 7]
        tm.assert_frame_equal(result, expected)

        result = df.rolling(window="3s", min_periods=1).add()
        expected = df.copy()
        expected["B"] = [0.0, 1, 3, 5, 7]
        tm.assert_frame_equal(result, expected)

        result = df.rolling(window="3s").add()
        expected = df.copy()
        expected["B"] = [0.0, 1, 3, 5, 7]
        tm.assert_frame_equal(result, expected)

        result = df.rolling(window="4s", min_periods=1).add()
        expected = df.copy()
        expected["B"] = [0.0, 1, 3, 6, 9]
        tm.assert_frame_equal(result, expected)

        result = df.rolling(window="4s", min_periods=3).add()
        expected = df.copy()
        expected["B"] = [np.nan, np.nan, 3, 6, 9]
        tm.assert_frame_equal(result, expected)

        result = df.rolling(window="5s", min_periods=1).add()
        expected = df.copy()
    ...
    def test_rolling_on_multi_index_level(self):
        ...
        result = df.rolling("10d", on=df.index.get_level_values("date")).add()
        expected = DataFrame(
