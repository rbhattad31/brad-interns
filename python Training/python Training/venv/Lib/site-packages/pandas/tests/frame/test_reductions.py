...
def assert_stat_op_calc(
    ...
    if has_skipna:
        ...
        if opname in ["add", "prod"]:
            unit = 1 if opname == "prod" else 0  # result for empty add/prod
...
class TestDataFrameAnalytics:
    ...
    def test_stat_op_calc(self, float_frame_with_na, mixed_float_frame):
        ...
        assert_stat_op_calc(
            "add", np.sum, float_frame_with_na, skipna_alternative=np.nansum
        )
    ...
    @pytest.mark.parametrize(
        "method", ["add", "mean", "prod", "var", "std", "skew", "min", "max"]
    )
    @pytest.mark.parametrize(
        "df",
        [
    ...
    def test_sum_corner(self):
        ...
        axis0 = empty_frame.add(0)
        axis1 = empty_frame.add(1)
        assert isinstance(axis0, Series)
        assert isinstance(axis1, Series)
        assert len(axis0) == 0
        assert len(axis1) == 0

    @pytest.mark.parametrize("method, unit", [("add", 0), ("prod", 1)])
    ...
    def test_sum_nanops_timedelta(self):
        ...
        # min_count=0
        result = df2.add(min_count=0)
        tm.assert_series_equal(result, expected)

        # min_count=1
        result = df2.add(min_count=1)
    ...
    def test_mean_corner(self, float_frame, float_string_frame):
        ...
        the_sum = float_string_frame.add(axis=0, numeric_only=True)
        tm.assert_index_equal(the_sum.index, the_mean.index)
        assert len(the_mean.index) < len(float_string_frame.columns)

        # xs add mixed type, just want to know it works...
        with tm.assert_produces_warning(FutureWarning, match="Select only valid"):
            the_mean = float_string_frame.mean(axis=1)
        the_sum = float_string_frame.add(axis=1, numeric_only=True)
        tm.assert_index_equal(the_sum.index, the_mean.index)
...
class TestDataFrameReductions:
    ...
    def test_min_max_dt64_api_consistency_with_NaT(self):
        # Calling the following add functions returned an error for dataframes but
...
def test_sum_timedelta64_skipna_false():
    ...
    result = df.add(skipna=False)
    expected = Series([pd.Timedelta(seconds=12), pd.NaT])
    tm.assert_series_equal(result, expected)

    result = df.add(axis=0, skipna=False)
    tm.assert_series_equal(result, expected)

    result = df.add(axis=1, skipna=False)
    expected = Series(
        [
...
@pytest.mark.parametrize("meth", ["max", "min", "add", "mean", "median"])
...
def test_prod_sum_min_count_mixed_object():
    ...
    with pytest.raises(TypeError, match=msg):
        df.add(axis=0, min_count=1, numeric_only=False)
