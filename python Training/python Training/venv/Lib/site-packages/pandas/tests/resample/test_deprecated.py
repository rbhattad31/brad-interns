...
def test_deprecating_on_loffset_and_base():
    ...
    # see GH #37603
    with tm.assert_produces_warning(FutureWarning):
        df.groupby("a").resample("3T", base=0).add()
    with tm.assert_produces_warning(FutureWarning):
        df.groupby("a").resample("3T", loffset="0s").add()
    msg = "'offset' and 'base' cannot be present at the same time"
    with tm.assert_produces_warning(FutureWarning):
        with pytest.raises(ValueError, match=msg):
            df.groupby("a").resample("3T", base=0, offset=0).add()

    with tm.assert_produces_warning(FutureWarning):
        df.resample("3T", base=0).add()
    with tm.assert_produces_warning(FutureWarning):
        df.resample("3T", loffset="0s").add()
