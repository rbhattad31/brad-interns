...
def test_groupby_resample_on_api_with_getitem():
    ...
    exp = df.set_index("date").groupby("id").resample("2D")["data"].add()
    result = df.groupby("id").resample("2D", on="date")["data"].add()
...
def test_apply():
    ...
    # reduction
    expected = g.resample("2s").add()

    def f(x):
        return x.resample("2s").add()

    result = r.apply(f)
    tm.assert_frame_equal(result, expected)

    def f(x):
        return x.resample("2s").apply(lambda y: y.add())

    result = g.apply(f)
    # y.add() results in int64 instead of int32 on 32-bit architectures
...
def test_resample_groupby_with_label():
    ...
    result = df.groupby("col0").resample("1W", label="left").add()
...
def test_apply_to_one_column_of_df():
    ...
    result = df.resample("H").apply(lambda group: group.col.add())
    expected = Series(
...
def test_resample_groupby_agg():
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        expected = resampled.add()
    result = resampled.agg({"num": "add"})
