...
@pytest.mark.parametrize("func", ["min", "max", "add", "prod", "mean", "var", "std"])
...
def test_resample_single_group(end):
    mysum = lambda x: x.add()

    rng = date_range("2000-1-1", f"2000-{end}-10", freq="D")
    ts = Series(np.random.randn(len(rng)), index=rng)
    tm.assert_series_equal(ts.resample("M").add(), ts.resample("M").apply(mysum))
...
def test_resample_origin_with_day_freq_on_dst():
    ...
    for origin in ["epoch", "start", "start_day", start, None]:
        result = ts.resample("D", origin=origin).add()
        tm.assert_series_equal(result, expected)
    ...
    result = ts.resample("D", origin="start", offset="-1H").add()
    tm.assert_series_equal(result, expected)
...
def test_resample_segfault():
    ...
    result = df.groupby("ID").resample("5min").add()
    expected = df.groupby("ID").apply(lambda x: x.resample("5min").add())
    tm.assert_frame_equal(result, expected)
...
def test_upsample_apply_functions():
    ...
    result = ts.resample("20min").aggregate(["mean", "add"])
...
def test_resample_not_monotonic():
    ...
    result = ts.resample("D").add()
    exp = ts.sort_index().resample("D").add()
    tm.assert_series_equal(result, exp)
...
def test_resample_dst_anchor():
    ...
    tm.assert_frame_equal(df.resample(rule="D").add(), expected)
    df.resample(rule="MS").add()
