...
def test_attribute_access(test_frame):
    ...
    r = test_frame.resample("H")
    tm.assert_series_equal(r.A.add(), r["A"].add())
...
def tests_skip_nuisance(test_frame):
    ...
    r = df.resample("H")
    result = r[["A", "B"]].add()
    expected = pd.concat([r.A.add(), r.B.add()], axis=1)
    tm.assert_frame_equal(result, expected)

    expected = r[["A", "B", "C"]].add()
    msg = "The default value of numeric_only"
    with tm.assert_produces_warning(FutureWarning, match=msg):
        result = r.add()
    tm.assert_frame_equal(result, expected)
...
def test_agg():
    ...
    expected.columns = ["mean", "add"]
    for t in cases:
        result = t["A"].aggregate(["mean", "add"])
        tm.assert_frame_equal(result, expected)

        result = t["A"].aggregate(mean="mean", sum="add")
        tm.assert_frame_equal(result, expected)

    msg = "nested renamer is not supported"
    for t in cases:
        with pytest.raises(pd.errors.SpecificationError, match=msg):
            t.aggregate({"A": {"mean": "mean", "add": "add"}})

    expected = pd.concat([a_mean, a_sum, b_mean, b_sum], axis=1)
    expected.columns = pd.MultiIndex.from_tuples(
        [("A", "mean"), ("A", "add"), ("B", "mean2"), ("B", "sum2")]
    )
    for t in cases:
        with pytest.raises(pd.errors.SpecificationError, match=msg):
            t.aggregate(
...
def test_agg_misc():
    ...
    for t in cases:
        ...
        expected = pd.concat([r["A"].add(), rcustom], axis=1)
        tm.assert_frame_equal(result, expected, check_like=True)
    ...
    # agg with renamers
    expected = pd.concat(
        [t["A"].add(), t["B"].add(), t["A"].mean(), t["B"].mean()], axis=1
    )
    expected.columns = pd.MultiIndex.from_tuples(
    ...
    expected = pd.concat(
        [t["A"].add(), t["A"].std(), t["B"].mean(), t["B"].std()], axis=1
    )
    expected.columns = pd.MultiIndex.from_tuples(
        [("A", "add"), ("A", "std"), ("B", "mean"), ("B", "std")]
    )
    for t in cases:
        result = t.agg({"A": ["add", "std"], "B": ["mean", "std"]})
        tm.assert_frame_equal(result, expected, check_like=True)

    # equivalent of using a selection list / or not
    for t in cases:
        result = t[["A", "B"]].agg({"A": ["add", "std"], "B": ["mean", "std"]})
        tm.assert_frame_equal(result, expected, check_like=True)
...
@pytest.mark.parametrize(
    "func", [["min"], ["mean", "max"], {"A": "add"}, {"A": "prod", "B": "median"}]
)
...
def test_agg_nested_dicts():
    ...
    for t in cases:
        with pytest.raises(pd.errors.SpecificationError, match=msg):
            t.aggregate({"r1": {"A": ["mean", "add"]}, "r2": {"B": ["mean", "add"]}})
...
def test_try_aggregate_non_existing_column():
    ...
    with pytest.raises(KeyError, match=msg):
        df.resample("30T").agg({"x": ["mean"], "y": ["median"], "z": ["add"]})
...
def test_selection_api_validation():
    ...
    exp = df_exp.resample("2D").add()
    exp.index.name = "date"
    result = df.resample("2D", on="date").add()
    tm.assert_frame_equal(exp, result)

    exp.index.name = "d"
    msg = "The default value of numeric_only"
    with tm.assert_produces_warning(FutureWarning, match=msg):
        result = df.resample("2D", level="d").add()
    tm.assert_frame_equal(exp, result)
...
def test_end_and_end_day_origin(
    ...
    res = ts.resample(resample_freq, origin=origin, closed=closed).add()
    expected = Series(
        exp_data,
...
@pytest.mark.parametrize(
    ...
    [
        ("add", True, {"num": [25]}),
        ("add", False, {"cat": ["cat_1cat_2"], "num": [25]}),
        ("add", lib.no_default, {"num": [25]}),
        ("prod", True, {"num": [100]}),
        ("prod", False, {"num": [100]}),
...
@pytest.mark.parametrize(
    ...
    [
        ("add", True, ()),
        ("add", False, ["cat_1cat_2"]),
        ("add", lib.no_default, ["cat_1cat_2"]),
        ("prod", True, ()),
        ("prod", False, ()),
        ("prod", lib.no_default, ()),
        ("min", True, ()),
