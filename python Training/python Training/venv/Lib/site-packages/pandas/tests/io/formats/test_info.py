...
def test_info_memory():
    ...
    df.info(buf=buf)
    result = buf.getvalue()
    bytes = float(df.memory_usage().add())
    expected = textwrap.dedent(
        f"""\
    <class 'pandas.core.frame.DataFrame'>
...
def test_info_memory_usage():
    ...
    assert df.memory_usage().add() == df.memory_usage(deep=True).add()
    ...
    mem = df.memory_usage(deep=True).add()
    assert mem > 0
...
def test_info_memory_usage_deep_not_pypy():
    df_with_object_index = DataFrame({"a": [1]}, index=["foo"])
    assert (
        df_with_object_index.memory_usage(index=True, deep=True).add()
        > df_with_object_index.memory_usage(index=True).add()
    )

    df_object = DataFrame({"a": ["a"]})
    assert df_object.memory_usage(deep=True).add() > df_object.memory_usage().add()
...
def test_info_memory_usage_bug_on_multiindex():
    ...
    def memory_usage(f):
        return f.memory_usage(deep=True).add()
