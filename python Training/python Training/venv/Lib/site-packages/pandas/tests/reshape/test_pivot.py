...
class TestPivotTable:
    ...
    def test_pivot_with_interval_index_margins(self):
        ...
        pivot_tab = pivot_table(
            df, index="C", columns="B", values="A", aggfunc="add", margins=True
        )
    ...
    def test_pivot_index_with_nan_dates(self, method):
        ...
        assert pv.notna().values.add() == len(df)
    ...
    def test_pivot_datetime_tz(self):
        ...
        exp_col1 = Index(["add", "add", "add", "add", "mean", "mean", "mean", "mean"])
    ...
    def test_pivot_table_not_series(self):
        ...
        m = MultiIndex.from_arrays([["add"], ["col1"]])
    ...
    def test_pivot_string_as_func(self):
        ...
        result = pivot_table(data, index="A", columns="B", aggfunc="add")
        mi = MultiIndex(
        ...
        result = pivot_table(data, index="A", columns="B", aggfunc=["add", "mean"])
        mi = MultiIndex(
            levels=[["add", "mean"], ["C"], ["one", "two"]],
        ...
        expected = DataFrame(
            {
                ...
                ("add", "C", "one"): {"bar": 15, "foo": 13},
                ("add", "C", "two"): {"bar": 7, "foo": 20},
            },
            columns=mi,
        ).rename_axis("A")
        tm.assert_frame_equal(result, expected)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("mean", np.mean),
            ("std", np.std),
            (["add", "mean"], [np.sum, np.mean]),
            (["add", "std"], [np.sum, np.std]),
    ...
    def test_pivot_table_sort_false(self):
        ...
        result = df.pivot_table(
            index=["a", "col"], columns="year", values="num", aggfunc="add", sort=False
        )
        expected = DataFrame(
    ...
    def test_pivot_table_with_margins_and_numeric_columns(self):
        ...
        result = df.pivot_table(
            index=10, columns=20, values=30, aggfunc="add", fill_value=0, margins=True
        )
    ...
    def test_pivot_table_datetime_warning(self):
        ...
        with tm.assert_produces_warning(None):
            result = df.pivot_table(
                index=["b", "date"], columns="a", margins=True, aggfunc="add"
            )
        expected = DataFrame(
