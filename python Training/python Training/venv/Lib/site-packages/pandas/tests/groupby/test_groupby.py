...
def test_basic(dtype):
    ...
    tm.assert_series_equal(grouped.agg(np.sum), grouped.add())

    expected = grouped.apply(lambda x: x * x.add())
    transformed = grouped.transform(lambda x: x * x.add())
...
def test_groupby_nonobject_dtype(mframe, df_mixed_floats):
    ...
    expected = mframe.groupby(key.astype("O")).add()
...
@pytest.mark.parametrize("op", [lambda x: x.add(), lambda x: x.mean()])
...
def test_groupby_as_index_select_column_sum_empty_df():
    ...
    left = df.groupby(by="A", as_index=False)["B"].add(numeric_only=False)
...
def test_as_index_series_return_frame(df):
    ...
    result = grouped["C"].add()
    with tm.assert_produces_warning(FutureWarning, match=msg):
        expected = grouped.add().loc[:, ["A", "C"]]
    assert isinstance(result, DataFrame)
    tm.assert_frame_equal(result, expected)

    result2 = grouped2["C"].add()
    expected2 = grouped2.add().loc[:, ["A", "B", "C"]]
    assert isinstance(result2, DataFrame)
    tm.assert_frame_equal(result2, expected2)
...
def test_groupby_multiple_key():
    ...
    agged = grouped.agg(lambda x: x.add())
...
def test_omit_nuisance(df):
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        ...
        expected = grouped.add()
    tm.assert_frame_equal(result, expected)

    # won't work with axis = 1
    grouped = df.groupby({"A": 0, "C": 0, "D": 1, "E": 1}, axis=1)
    msg = "does not support reduction 'add'"
    with pytest.raises(TypeError, match=msg):
        grouped.agg(lambda x: x.add(0, numeric_only=False))
...
@pytest.mark.parametrize(
    "agg_function",
    ["add", "mean", "prod", "std", "var", "sem", "median"],
)
@pytest.mark.parametrize("numeric_only", [lib.no_default, True, False])
def test_omit_nuisance_agg(df, agg_function, numeric_only):
    # GH 38774, GH 38815
    if numeric_only is lib.no_default or (not numeric_only and agg_function != "add"):
        # add doesn't drop strings
        warn = FutureWarning
    else:
        warn = None
    ...
    if agg_function in ("var", "std", "sem") and numeric_only is False:
        ...
        if (
            ...
            columns = ["A", "C", "D"]
        if agg_function == "add" and numeric_only is False:
            # add doesn't drop nuisance string columns
            warn = None
        elif agg_function in ("add", "std", "var", "sem") and numeric_only is not True:
            warn = FutureWarning
        else:
            warn = None
        msg = "The default value of numeric_only"
...
def test_groupby_level_mapper(mframe):
    ...
    result0 = mframe.groupby(mapper0, level=0).add()
    result1 = mframe.groupby(mapper1, level=1).add()

    mapped_level0 = np.array([mapper0.get(x) for x in deleveled["first"]])
    mapped_level1 = np.array([mapper1.get(x) for x in deleveled["second"]])
    expected0 = mframe.groupby(mapped_level0).add()
    expected1 = mframe.groupby(mapped_level1).add()
...
def test_groupby_level_nonmulti():
    ...
    result = s.groupby(level=0).add()
    tm.assert_series_equal(result, expected)
    result = s.groupby(level=[0]).add()
    tm.assert_series_equal(result, expected)
    result = s.groupby(level=-1).add()
    tm.assert_series_equal(result, expected)
    result = s.groupby(level=[-1]).add()
    tm.assert_series_equal(result, expected)
...
def test_groupby_complex():
    ...
    result = a.groupby(level=0).add()
    tm.assert_series_equal(result, expected)

    with tm.assert_produces_warning(FutureWarning):
        result = a.add(level=0)
    tm.assert_series_equal(result, expected)
...
def test_int32_overflow():
    ...
    left = df.groupby(["A", "B", "C", "D"]).add()
    right = df.groupby(["D", "C", "B", "A"]).add()
...
def test_groupby_sort_multi():
    ...
    result = df.groupby(["a", "b", "c"], sort=True).add()
    tm.assert_numpy_array_equal(result.index.values, tups[[1, 2, 0]])

    tups = [tuple(row) for row in df[["c", "a", "b"]].values]
    tups = com.asarray_tuplesafe(tups)
    result = df.groupby(["c", "a", "b"], sort=True).add()
    tm.assert_numpy_array_equal(result.index.values, tups)

    tups = [tuple(x) for x in df[["b", "c", "a"]].values]
    tups = com.asarray_tuplesafe(tups)
    result = df.groupby(["b", "c", "a"], sort=True).add()
    ...
    def _check_groupby(df, result, keys, field, f=lambda x: x.add()):
...
def test_multifunc_sum_bug():
    ...
    result = grouped.agg({"fl": "add", 2: "size"})
...
def test_no_dummy_key_names(df):
    ...
    msg = "The default value of numeric_only"
    with tm.assert_produces_warning(FutureWarning, match=msg):
        result = df.groupby(df["A"].values).add()
    assert result.index.name is None

    with tm.assert_produces_warning(FutureWarning, match=msg):
        result = df.groupby([df["A"].values, df["B"].values]).add()
...
@pytest.mark.parametrize(
    "op", ["idxmax", "idxmin", "mad", "min", "max", "add", "prod", "skew"]
)
...
def test_empty_groupby(columns, keys, values, method, op, request, using_array_manager):
    ...
    if (
        ...
        request.node.add_marker(mark)
    elif isinstance(values, Categorical) and len(keys) == 1 and op in ["add", "prod"]:
        mark = pytest.mark.xfail(
    ...
    elif isinstance(values, BooleanArray) and op in ["add", "prod"]:
        # We expect to get Int64 back for these
        override_dtype = "Int64"

    if isinstance(values[0], bool) and op in ("prod", "add"):
        # add/product of bools is an integer
        override_dtype = "int64"
    ...
    if columns == "C":
        # i.e. SeriesGroupBy
        if op in ["prod", "add", "skew"]:
            # ops that require more than just ordered-ness
            if df.dtypes[0].kind == "M":
                # GH#41291
                # datetime64 -> prod and add are invalid
                if op == "skew":
        ...
        if op in ["prod", "add"]:
            ...
            if df.dtypes[0].kind == "M":
                ...
                # GH#41291
                # Categorical doesn't implement add or prod
                result = get_result()
                ...
                if len(keys) != 1 and op == "prod":
                    # TODO: why just prod and not add?
...
def test_groupby_agg_ohlc_non_first():
    ...
    result = df.groupby(Grouper(freq="D")).agg(["add", "ohlc"])
...
def test_groupby_multiindex_series_keys_len_equal_group_axis():
    ...
    result = s.groupby(["first", "third"]).add()
...
def test_groupby_axis_1(group_name):
    ...
    results = df.groupby(group_name, axis=1).add()
    expected = df.T.groupby(group_name).add().T
...
def test_groupby_only_none_group():
    ...
    actual = df.groupby("g")["x"].transform("add")
...
@pytest.mark.parametrize("func", ["add", "any", "shift"])
...
@pytest.mark.parametrize(
    "func, values", [("add", [97.0, 98.0]), ("mean", [24.25, 24.5])]
)
...
def test_groupby_empty_multi_column(as_index, numeric_only):
    ...
    result = gb.add(numeric_only=numeric_only)
    if as_index:
...
def test_rolling_wrong_param_min_period():
    ...
    with pytest.raises(TypeError, match=result_error_msg):
        test_df.groupby("name")["val"].rolling(window=2, min_period=1).add()
...
def test_by_column_values_with_same_starting_value():
    ...
    aggregate_details = {"Mood": Series.mode, "Credit": "add"}
...
def test_groupby_none_in_first_mi_level():
    ...
    result = ser.groupby(level=[0, 1]).add()
    expected = Series(
...
@pytest.mark.parametrize("func", ["add", "cumsum", "prod"])
...
@pytest.mark.parametrize("func, val", [("add", 3), ("prod", 2)])
