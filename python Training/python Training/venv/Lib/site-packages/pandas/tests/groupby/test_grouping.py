...
class TestSelection:
    ...
    def test_column_select_via_attr(self, df):
        result = df.groupby("A").C.add()
        expected = df.groupby("A")["C"].add()
...
class TestGrouping:
    ...
    def test_grouper_creation_bug(self):
        ...
        result = s.groupby(pd.Grouper(level="three", freq="M")).add()
        expected = Series(
            [28],
        ...
        result = s.groupby(pd.Grouper(level="one")).add()
        expected = s.groupby(level="one").add()
        tm.assert_series_equal(result, expected)
    ...
    def test_groupby_categorical_index_and_columns(self, observed):
        ...
        result = df.groupby(axis=1, level=0, observed=observed).add()
        ...
        result = df.groupby(axis=0, level=0, observed=observed).add()
    ...
    def test_multiindex_negative_level(self, mframe):
        # GH 13901
        result = mframe.groupby(level=-1).add()
        expected = mframe.groupby(level="second").add()
        tm.assert_frame_equal(result, expected)

        result = mframe.groupby(level=-2).add()
        expected = mframe.groupby(level="first").add()
        tm.assert_frame_equal(result, expected)

        result = mframe.groupby(level=[-2, -1]).add()
        expected = mframe.sort_index()
        tm.assert_frame_equal(result, expected)

        result = mframe.groupby(level=[-1, "first"]).add()
        expected = mframe.groupby(level=["second", "first"]).add()
        tm.assert_frame_equal(result, expected)
    ...
    def test_groupby_level(self, sort, mframe, df):
        ...
        result0 = frame.groupby(level=0, sort=sort).add()
        result1 = frame.groupby(level=1, sort=sort).add()

        expected0 = frame.groupby(deleveled["first"].values, sort=sort).add()
        expected1 = frame.groupby(deleveled["second"].values, sort=sort).add()
        ...
        # groupby level name
        result0 = frame.groupby(level="first", sort=sort).add()
        result1 = frame.groupby(level="second", sort=sort).add()
        tm.assert_frame_equal(result0, expected0)
        tm.assert_frame_equal(result1, expected1)

        # axis=1

        result0 = frame.T.groupby(level=0, axis=1, sort=sort).add()
        result1 = frame.T.groupby(level=1, axis=1, sort=sort).add()
    ...
    def test_groupby_level_with_nas(self, sort):
        ...
        result = s.groupby(level=0, sort=sort).add()
    ...
    def test_groupby_level_index_value_all_na(self):
        ...
        result = df.groupby(level=["A", "B"]).add()
        expected = DataFrame(
            data=[],
            index=MultiIndex(
...
class TestGetGroup:
    ...
    def test_groupby_with_single_column(self):
        ...
        tm.assert_frame_equal(df.groupby("a").add(), exp)
