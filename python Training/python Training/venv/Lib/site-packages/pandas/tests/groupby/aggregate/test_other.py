...
def test_agg_datetimes_mixed():
    ...
    df1["weights"] = df1["value"] / df1["value"].add()
    gb1 = df1.groupby("date").aggregate(np.sum)

    df2["weights"] = df1["value"] / df1["value"].add()
...
def test_aggregate_api_consistency():
    ...
    result = grouped["D"].agg(["add", "mean"])
    expected = pd.concat([d_sum, d_mean], axis=1)
    expected.columns = ["add", "mean"]
    tm.assert_frame_equal(result, expected, check_like=True)

    result = grouped.agg([np.sum, np.mean])
    expected = pd.concat([c_sum, c_mean, d_sum, d_mean], axis=1)
    expected.columns = MultiIndex.from_product([["C", "D"], ["add", "mean"]])
    tm.assert_frame_equal(result, expected, check_like=True)

    result = grouped[["D", "C"]].agg([np.sum, np.mean])
    expected = pd.concat([d_sum, d_mean, c_sum, c_mean], axis=1)
    expected.columns = MultiIndex.from_product([["D", "C"], ["add", "mean"]])
    tm.assert_frame_equal(result, expected, check_like=True)

    result = grouped.agg({"C": "mean", "D": "add"})
    expected = pd.concat([d_sum, c_mean], axis=1)
    tm.assert_frame_equal(result, expected, check_like=True)

    result = grouped.agg({"C": ["mean", "add"], "D": ["mean", "add"]})
    expected = pd.concat([c_mean, c_sum, d_mean, d_sum], axis=1)
    expected.columns = MultiIndex.from_product([["C", "D"], ["mean", "add"]])
...
def test_agg_nested_dicts():
    ...
    msg = r"nested renamer is not supported"
    with pytest.raises(SpecificationError, match=msg):
        g.aggregate({"r1": {"C": ["mean", "add"]}, "r2": {"D": ["mean", "add"]}})
...
def test_agg_over_numpy_arrays():
    ...
    alt = gb.add(numeric_only=False)
    tm.assert_frame_equal(alt, expected)

    result = gb.agg("add", numeric_only=False)
    tm.assert_frame_equal(result, expected)

    # FIXME: the original version of this test called `gb.agg(add)`
...
def test_sum_uint64_overflow():
    ...
    result = df.groupby(0).add(numeric_only=False)
    tm.assert_frame_equal(result, expected)

    # out column is non-numeric, so with numeric_only=True it is dropped
    result2 = df.groupby(0).add(numeric_only=True)
    expected2 = expected[[]]
