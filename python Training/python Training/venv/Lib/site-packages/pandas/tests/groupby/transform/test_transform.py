...
def test_transform_with_non_scalar_group():
    ...
    with pytest.raises(ValueError, match=msg):
        df.groupby(axis=1, level=1).transform(lambda z: z.div(z.add(axis=1), axis=0))
...
def test_categorical_and_not_categorical_key(observed):
    ...
    # DataFrame case
    result = df_with_categorical.groupby(["A", "C"], observed=observed).transform("add")
    expected = df_without_categorical.groupby(["A", "C"]).transform("add")
    tm.assert_frame_equal(result, expected)
    ...
    expected = df_without_categorical.groupby(["A", "C"])["B"].transform("add")
    tm.assert_series_equal(result, expected)
