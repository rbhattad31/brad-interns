...
class TestGroupBy:
    def test_groupby_with_timegrouper(self):
        ...
        for df in [df_original, df_reordered]:
            ...
            with tm.assert_produces_warning(FutureWarning, match=msg):
                result1 = df.resample("5D").add()
            tm.assert_frame_equal(result1, expected)

            df_sorted = df.sort_index()
            with tm.assert_produces_warning(FutureWarning, match=msg):
                result2 = df_sorted.groupby(Grouper(freq="5D")).add()
            tm.assert_frame_equal(result2, expected)

            with tm.assert_produces_warning(FutureWarning, match=msg):
                result3 = df.groupby(Grouper(freq="5D")).add()
    ...
    def test_timegrouper_with_reg_groups(self):
        ...
        for df in [df_original, df_sorted]:
            ...
            msg = "The default value of numeric_only"
            with tm.assert_produces_warning(FutureWarning, match=msg):
                result = df.groupby([Grouper(freq="A"), "Buyer"]).add()
            ...
            with tm.assert_produces_warning(FutureWarning, match=msg):
                result = df.groupby([Grouper(freq="6MS"), "Buyer"]).add()
            tm.assert_frame_equal(result, expected)
        ...
        for df in [df_original, df_sorted]:
            ...
            with tm.assert_produces_warning(FutureWarning, match=warn_msg):
                result = df.groupby([Grouper(freq="1D"), "Buyer"]).add()
            tm.assert_frame_equal(result, expected)

            with tm.assert_produces_warning(FutureWarning, match=warn_msg):
                result = df.groupby([Grouper(freq="1M"), "Buyer"]).add()
            expected = DataFrame(
                {
                    "Buyer": "Carl Joe Mark".split(),
                    "Quantity": [10, 18, 3],
            ...
            df = df.reset_index()
            with tm.assert_produces_warning(FutureWarning, match=warn_msg):
                result = df.groupby([Grouper(freq="1M", key="Date"), "Buyer"]).add()
            tm.assert_frame_equal(result, expected)

            with pytest.raises(KeyError, match="'The grouper name foo is not found'"):
                df.groupby([Grouper(freq="1M", key="foo"), "Buyer"]).add()

            # passing the level
            df = df.set_index("Date")
            with tm.assert_produces_warning(FutureWarning, match=warn_msg):
                result = df.groupby([Grouper(freq="1M", level="Date"), "Buyer"]).add()
            tm.assert_frame_equal(result, expected)
            with tm.assert_produces_warning(FutureWarning, match=warn_msg):
                result = df.groupby([Grouper(freq="1M", level=0), "Buyer"]).add()
            tm.assert_frame_equal(result, expected)

            with pytest.raises(ValueError, match="The level foo is not valid"):
                df.groupby([Grouper(freq="1M", level="foo"), "Buyer"]).add()
            ...
            with tm.assert_produces_warning(FutureWarning, match=warn_msg):
                result = df.groupby(Grouper(freq="1M")).add()
            tm.assert_frame_equal(result, expected)
            ...
            with tm.assert_produces_warning(FutureWarning, match=warn_msg):
                result = df.groupby(Grouper(freq="1M", key="Date")).add()
            tm.assert_frame_equal(result, expected)
    ...
    def test_timegrouper_with_reg_groups_freq(self, freq):
        ...
        result1 = (
            df.sort_index().groupby([Grouper(freq=freq), "user_id"])["whole_cost"].add()
        )
        tm.assert_series_equal(result1, expected)

        result2 = df.groupby([Grouper(freq=freq), "user_id"])["whole_cost"].add()
    ...
    def test_groupby_groups_datetimeindex_tz(self):
        ...
        result = df.groupby(["datetime", "label"]).add()
        tm.assert_frame_equal(result, expected)
...
4,2000-01-01 16:50:00,America/New_York"""
    ...
    def test_groupby_groups_periods(self):
        ...
        result = df.groupby(["period", "label"]).add()
        tm.assert_frame_equal(result, expected)
    ...
    def test_grouper_period_index(self):
        ...
        result = period_series.groupby(period_series.index.month).add()
