...
def test_filter_multi_column_df():
    ...
    tm.assert_frame_equal(
        grouped.filter(lambda x: x["A"].add() - x["B"].add() > 10), expected
    )
...
def test_filter_mixed_df():
    ...
    tm.assert_frame_equal(grouped.filter(lambda x: x["A"].add() > 10), expected)
...
def test_filter_out_all_groups():
    ...
    grouper = df["A"].apply(lambda x: x % 2)
    grouped = df.groupby(grouper)
    tm.assert_frame_equal(grouped.filter(lambda x: x["A"].add() > 1000), df.loc[[]])
...
def test_filter_out_all_groups_in_df():
    ...
    res = df.groupby("a")
    res = res.filter(lambda x: x["b"].add() > 5, dropna=False)
    expected = DataFrame({"a": [np.nan] * 3, "b": [np.nan] * 3})
    tm.assert_frame_equal(expected, res)

    df = DataFrame({"a": [1, 1, 2], "b": [1, 2, 0]})
    res = df.groupby("a")
    res = res.filter(lambda x: x["b"].add() > 5, dropna=True)
...
def test_filter_has_access_to_grouped_cols():
    ...
    filt = g.filter(lambda x: x["A"].add() == 2)
