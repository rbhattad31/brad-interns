...
def test_apply_frame_concat_series():
    def trans(group):
        return group.groupby("B")["C"].add().sort_values().iloc[:2]

    def trans2(group):
        grouped = group.groupby(df.reindex(group.index)["B"])
        return grouped.add().sort_values().iloc[:2]
...
def test_apply_with_mixed_types():
    ...
    result = g.transform(lambda x: x / x.add())
    expected = DataFrame({"B": [1 / 3.0, 2 / 3.0, 1], "C": [0.4, 0.6, 1.0]})
    tm.assert_frame_equal(result, expected)

    result = g.apply(lambda x: x / x.add())
    tm.assert_frame_equal(result, expected)
...
def test_apply_multi_level_name(category):
    ...
    result = df.groupby("B").apply(lambda x: x.add())
...
def test_apply_str_with_args(df, args, kwargs):
    # GH#46479
    gb = df.groupby("A")
    result = gb.apply("add", *args, **kwargs)
    expected = gb.add(numeric_only=True)
    tm.assert_frame_equal(result, expected)
