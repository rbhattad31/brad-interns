...
def test_groupby_aggregation_multi_level_column():
    ...
    result = gb.add(numeric_only=False)
...
def test_agg_apply_corner(ts, tsframe):
    ...
    tm.assert_series_equal(grouped.add(), exp)
    ...
    tm.assert_frame_equal(grouped.add(), exp_df)
...
@pytest.mark.parametrize(
    ...
    [
        ("add", [[2, 4], [10, 12], [18, 20]], {10: "int64", 20: "int64"}),
...
def test_multi_function_flexible_mix(df):
    ...
    # Expected
    d = {"C": {"foo": "mean", "bar": "std"}, "D": {"add": "add"}}
    # this uses column selection & renaming
    msg = r"nested renamer is not supported"
    with pytest.raises(SpecificationError, match=msg):
        grouped.aggregate(d)
...
def test_bool_agg_dtype(op):
    # GH 7001
    # Bool add aggregations result in int
...
def test_order_aggregate_multiple_funcs():
    ...
    res = df.groupby("A").agg(["add", "max", "mean", "ohlc", "min"])
    result = res.columns.levels[1]

    expected = Index(["add", "max", "mean", "ohlc", "min"])
...
class TestNamedAggregationSeries:
    def test_series_named_agg(self):
        ...
        result = gr.agg(a="add", b="min")
        expected = DataFrame(
            {"a": [3, 7], "b": [1, 3]}, columns=["a", "b"], index=[0, 1]
        )
        tm.assert_frame_equal(result, expected)

        result = gr.agg(b="min", a="add")
...
class TestNamedAggregationDataFrame:
    ...
    def test_missing_raises(self):
        ...
        with pytest.raises(KeyError, match=match):
            df.groupby("A").agg(c=("C", "add"))

    def test_agg_namedtuple(self):
        df = DataFrame({"A": [0, 1], "B": [1, 2]})
        result = df.groupby("A").agg(
            b=pd.NamedAgg("B", "add"), c=pd.NamedAgg(column="B", aggfunc="count")
        )
        expected = df.groupby("A").agg(b=("B", "add"), c=("B", "count"))
        tm.assert_frame_equal(result, expected)
...
class TestLambdaMangling:
    ...
    def test_with_kwargs(self):
        f1 = lambda x, y, b=1: x.add() + y + b
        f2 = lambda x, y, b=2: x.add() + y * b
...
@pytest.mark.parametrize(
    "func, output", [("mean", [8 + 18j, 10 + 22j]), ("add", [40 + 90j, 50 + 110j])]
)
...
@pytest.mark.parametrize(
    "func", [["min"], ["mean", "max"], {"b": "add"}, {"b": "prod", "c": "median"}]
)
