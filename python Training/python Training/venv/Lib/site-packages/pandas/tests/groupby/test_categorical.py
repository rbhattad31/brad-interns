...
def test_categorical_index():
    ...
    # with a cat index
    result = df.set_index("cats").groupby(level=0, observed=False).add()
    expected = df[list("abcd")].groupby(cats.codes, observed=False).add()
    expected.index = CategoricalIndex(
    ...
    result = df.groupby("cats", observed=False).add()
    expected = df[list("abcd")].groupby(cats.codes, observed=False).add()
    expected.index = CategoricalIndex(
        Categorical.from_codes([0, 1, 2, 3], levels, ordered=True), name="cats"
    )
...
def test_as_index():
    ...
    result = df.groupby(["cat", "A"], as_index=False, observed=True).add()
    expected = DataFrame(
        {
    ...
    result = df.groupby(["cat", f], as_index=False, observed=True).add()
    expected = DataFrame(
        {
            "cat": Categorical([1, 2], categories=df.cat.cat.categories),
            "A": [10, 22],
    ...
    result = df.groupby(["cat", s], as_index=False, observed=True).add()
    tm.assert_frame_equal(result, expected)
    ...
    for name in [None, "X", "B"]:
        ...
        result = df.groupby(group_columns, as_index=False, observed=True).add()
...
def test_empty_sum():
    ...
    # 0 by default
    result = df.groupby("A", observed=False).B.add()
    expected = Series([3, 1, 0], expected_idx, name="B")
    tm.assert_series_equal(result, expected)

    # min_count=0
    result = df.groupby("A", observed=False).B.add(min_count=0)
    expected = Series([3, 1, 0], expected_idx, name="B")
    tm.assert_series_equal(result, expected)

    # min_count=1
    result = df.groupby("A", observed=False).B.add(min_count=1)
    expected = Series([3, 1, np.nan], expected_idx, name="B")
    tm.assert_series_equal(result, expected)

    # min_count>1
    result = df.groupby("A", observed=False).B.add(min_count=2)
...
def test_groupby_agg_observed_true_single_column(as_index, expected):
    ...
    result = df.groupby(["a", "b"], as_index=as_index, observed=True)["x"].add()
...
def test_series_groupby_on_2_categoricals_unobserved_zeroes_or_nans(
    ...
    # Except for .add(). If the observed categories add to dtype=float (i.e. their
    # sums have decimals), then the zeros for the missing categories should also be
    # floats.
    if zero_or_nan == 0 and reduction_func != "add":
...
def test_aggregate_categorical_with_isnan():
    ...
    result = df.groupby(["A", "B"]).agg(lambda df: df.isna().add())
