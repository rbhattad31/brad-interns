...
class Dim2CompatTests(BaseExtensionTests):
    ...
    @pytest.mark.parametrize("method", ["mean", "median", "var", "std", "add", "prod"])
    ...
    def test_reductions_2d_axis0(self, data, method):
        ...
        if method in ["mean", "median", "add", "prod"]:
            # std and var are not dtype-preserving
            expected = data
            if method in ["add", "prod"] and data.dtype.kind in "iub":
                dtype = get_reduction_result_dtype(data.dtype)

                expected = data.astype(dtype)
                if data.dtype.kind == "b" and method in ["add", "prod"]:
