...
def average_day_sales(sales_register_df, main_config):
    ...
    try:
        ...
        logging.error("Exception occurred while creating month column from billing date that be due to values other than dates")
        raise average_day_sales_Exception(exception_message)
    ...
    try:
        ...
        logging.error("Exception occurred while creating year column from billing date that be due to values other than dates")
        raise average_day_sales_Exception(exception_message)
    ...
    if quarter == q4_months:
        ...
        logging.error("The program could not find financial timeline information such as quarter, months and year of sales register")
        raise average_day_sales_Exception(exception_message)
    ...
    try:
        ...
        logging.error("Below exception occurred while creating year column from billing date that ay be due to values other than dates")
        raise average_day_sales_Exception(exception_message)
...
#    average_day_sales_pivot_df = average_day_sales_pivot_df.sort_values(by=["Concentration"], ascending=True)  # descending order
    ...
    #  date time
    average_day_sales_pivot_df["Billing Date"] = pd.to_datetime(average_day_sales_pivot_df["Billing Date"]).dt.strftime("%d-%b-%Y")
...
if __name__ == '__main__':
    ...
    str_sales_register_file_path = "sales register q1.xlsx"
    str_sales_register_file_sheet_name = "Sheet1"
    config_file_path = 'Config file.xlsx'
    config_file_sheet_name = 'Average_Day_Sales'

    try:
        dict_main_config = create_config_dict_from_Config_file(path=config_file_path, sheet_name=config_file_sheet_name)
        skip_rows = dict_main_config['sales_register_file_header_row_number'] - 1
    ...
    try:
        sales_register_df = pd.read_excel(str_sales_register_file_path, skiprows=skip_rows,
                                      sheet_name=str_sales_register_file_sheet_name)
        ...
        logging.error("Below exception occurred while reading sales register file")
        raise average_day_sales_Exception(exception_message)

    try:
        to = dict_main_config["to"]
        cc = dict_main_config["cc"]
        average_day_sales_output_file_path = average_day_sales(sales_register_df=sales_register_df, main_config=dict_main_config)
        ...
        send_mail_with_attachment(to=dict_main_config["to"], cc=dict_main_config["cc"], subject=subject, body=body, attachment_path=average_day_sales_output_file_path)
